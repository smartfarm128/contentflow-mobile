...
export function FallingPatternTemplate({ time, width, height, values }: HtmlTemplateProps) {
  // Read controls
  const color = String(values.color ?? "var(--primary)");
  const backgroundColor = String(values.backgroundColor ?? "var(--background)");
  const duration = Math.max(1, Number(values.duration ?? 150));
  const blurIntensity = String(values.blurIntensity ?? "1em");
  const density = Number(values.density ?? 1);
  const text = String(values.text ?? "Falling Pattern");
  ...
  // Parse start/end pairs once
  const startPairs = useMemo(() => {
    return startPositions.split(',').map((p) => {
      const parts = p.trim().split(' ');
      return { x: parts[0], y: parseFloat(parts[1]) };
    });
  }, []);

  const endPairs = useMemo(() => {
    return endPositions.split(',').map((p) => {
      const parts = p.trim().split(' ');
      return { x: parts[0], y: parseFloat(parts[1]) };
    });
  }, []);

  const progressInLoop = (time / duration) % 1.0;

  const currentPositions = useMemo(() => {
    return startPairs.map((start, i) => {
      const endY = endPairs[i].y;
      const currentY = start.y + (endY - start.y) * progressInLoop;
      return `${start.x} ${currentY}px`;
    }).join(', ');
  }, [startPairs, endPairs, progressInLoop]);
    ...
    >
      ...
      <div
        style={{
          ...
          backgroundPosition: currentPositions,
          zIndex: 0,
        }}
      />
