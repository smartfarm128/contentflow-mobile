...
export const MacOSDock: React.FC<MacOSDockProps> = ({ 
  ...
  const [mouseXState, setMouseXState] = useState<number | null>(null);
  const mouseX = mouseXOverride !== undefined ? mouseXOverride : mouseXState;
  ...
  // Animation loop / position updater
  const updateMagnificationAndPositions = useCallback(() => {
    const targetScales = calculateTargetMagnification(mouseX);
    const targetPositions = calculatePositions(targetScales);

    // If mouseXOverride is provided, do NOT use LERP (transition instantly to preserve playhead scrubbing)
  ...
  }, [mouseX, mouseXOverride, calculateTargetMagnification, calculatePositions]);

  // Animation frame effect
  useEffect(() => {
    if (mouseXOverride !== undefined) {
      updateMagnificationAndPositions();
      return;
    }
  ...
  }, [updateMagnificationAndPositions, mouseXOverride]);

  // Throttled mouse movement handler
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    if (mouseXOverride !== undefined) return;
    ...
    if (dockRef.current) {
      ...
      setMouseXState(e.clientX - rect.left - padding);
    }
  }, [baseIconSize, mouseXOverride]);
